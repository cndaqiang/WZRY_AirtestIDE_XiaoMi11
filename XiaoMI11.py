...
, intervalfunc=异常处理)
        ...
        for loop in range(looptimes):
            if exists(Template(r"tpl1685431840269.png", record_pos=(0.127, 0.187), resolution=(2400, 1080))):
                ...
                break
            sleep(1)
        for loop in range(looptimes*5):#等待时间太长
            # 确认匹配
            ...
            sleep(2)
        if True:
            for loop in range(looptimes*60*5):
                #选择英雄按钮
...
def 大厅中():
    if exists(Template(r"tpl1685442975719.png", record_pos=(-0.082, 0.116), resolution=(2400, 1080))):
        logger.warning("正在大厅中")
        return True


def 对战中():
    if exists(Template(r"tpl1685442267997.png", record_pos=(0.34, 0.163), resolution=(2400, 1080))):
